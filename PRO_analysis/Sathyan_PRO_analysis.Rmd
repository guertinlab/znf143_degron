...
for (chip.peak in Sys.glob(file.path("/Users/jinhongdong/Desktop/TSSinference/ZNF143dTag_2023/ChIP/quantile*summits.bed"))) {
    print(chip.peak)
...
df.cdf.quant.summits.rep = data.frame(matrix(nrow = 0, ncol = 3))   
colnames(df.cdf.quant.summits.rep) = c("V4", "dis", "status")
...
for (chip.peak in Sys.glob(file.path("/Users/jinhongdong/Desktop/TSSinference/ZNF143dTag_2023/ChIP/quantile*summits.bed"))) {
    ...
    df.all[,3] = x
    df.cdf.quant.summits.rep = rbind(df.cdf.quant.summits.rep, df.all)
    }

# had to use "rep.int" instead of "rep" because "rep" got overwritten by one of the sourced functions
plot_cdf(df.cdf.quant.summits.rep, tf = "ZNF143_summit_quantiles_all", cat = "Repressed", col.lines = c(rep.int("grey60",20),colorRampPalette(c("red","pink"))(13),colorRampPalette(c("blue", "light blue"))(7)), line.type = c(rep.int(2, 20), rep.int(1,20)), cex = 0.25)
```

ZNF143_summit_quantiles_all_cdf_Repressed.pdf (to png)
...
df.cdf.quant.summits = data.frame(matrix(nrow = 0, ncol = 3))   
colnames(df.cdf.quant.summits) = c("V4", "dis", "status")
...
for (chip.peak in Sys.glob(file.path("/Users/jinhongdong/Desktop/TSSinference/ZNF143dTag_2023/ChIP/quantile*summits.bed"))) {
    ...
    x = paste(as.character(df.all[,3]), quantile.name)
    df.all[,3] = x
    df.cdf.quant.summits = rbind(df.cdf.quant.summits, df.all)
    }

levels = levels(factor(df.cdf.quant.summits$status))
levels = c(levels[grepl("Matched", levels)], levels[!grepl("Matched", levels)])
df.cdf.quant.summits$status = factor(df.cdf.quant.summits$status, levels = levels)

# had to use "rep.int" instead of "rep" because "rep" got overwritten by one of the sourced functions
plot_cdf(df.cdf.quant.summits, tf = "ZNF143_summit_quantiles_all", cat = "Activated", col.lines = c(rep.int("grey60",20),colorRampPalette(c("red","pink"))(20)), line.type = c(rep.int(2, 20), rep.int(1,20)), cex = 0.25)
```
ZNF143_summit_quantiles_all_cdf_Activated.pdf (to png)
...
temp[temp$treatment == "Matched to Activated",]$treatment = "Unchanged"
temp[temp$treatment == "Matched to Repressed",]$treatment = "Unchanged"
...
ma.plot.lattice(ma.df, filename='ZNF143_dTAG_PRO_v4', title.main="Differential Expression", col=c("#ce228e" ,"#2290cf", "grey90"))
```
...
do
    ...
    echo $i
    echo $k
    cat qmerge${i}.bed quantile${k}.bed > qmerge$((i+1)).bed
done
```
...
plot_cdf <- function(df.all, tf="qmerge", cat = "Repressed", col.lines = c("#ce228e", "grey60", "#2290cf","grey90"), line.type = c(1), cex = 1) {
pdf(paste0(tf, "_ZNF143_cumulative_cdf_", cat, ".pdf"), width=6.2, height=3.83) 
         print(ecdfplot(~log(abs(dis), base = 10), groups = status, data = df.all,
         auto.key = list(lines=TRUE, points=FALSE, cex = cex, space="right"),
         col = col.lines,
         aspect = 1,
...
colorChunks=c(plasma(13)[1:11], brewer.pal(8,"Spectral"))

plot_cdf(df.cdf, tf = "qmerge", cat = cat, col.lines = c(rep("grey60",19),colorChunks), line.type = c(rep(2, 19), rep(1,19)), cex = 0.3)
```

[image here, might change colors]
qmerge_ZNF143_cumulative_cdf_Repressed.pdf
