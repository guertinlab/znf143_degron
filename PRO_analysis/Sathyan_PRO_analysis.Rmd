...
# Obtain FASTQ files from GEO

Log into the high-performance cluster, start an interactive node, set up the directories, and get the files from GEO accession record GSE266491 for PRO-seq files. 
...
The SRA runs for GSE266491 are SRR28879486 through SRR28879493.
...
#SBATCH --job-name=get_SRA_SRR288794XXX
#SBATCH -N 1
#SBATCH -n 1
#SBATCH -c 10
#SBATCH -p general
...
cd /scratch/jdong/ZNF143dTAG/PRO/FASTQ 
echo "Current wd:" $(pwd)
echo "Current node:" $(hostname)

module load sratoolkit/3.0.5 # latest version on Xanadu

fasterq-dump SRR288794XXX # replace XXX with number from 86-93 inclusive 
...
```{r engine='bash', eval=F, echo=TRUE}
file=/home/FCAM/jdong/script_templates/get_SRA_template.sh

for i in {86..93}
do
  echo $i
  sed -e "s/XXX/${i}/g" "$file" > get_SRA_SRR288794${i}.sh
  grep fasterq get_SRA_SRR288794${i}.sh
done
...
```{r engine='bash', eval=F, echo=TRUE}
mv SRR28879486_1.fastq HEK_CloneZD29_30min_dTAGV1_rep4_PE1.fastq
mv SRR28879486_2.fastq HEK_CloneZD29_30min_dTAGV1_rep4_PE2.fastq
mv SRR28879487_1.fastq HEK_CloneZD29_30min_dTAGV1_rep3_PE1.fastq
mv SRR28879487_2.fastq HEK_CloneZD29_30min_dTAGV1_rep3_PE2.fastq
mv SRR28879488_1.fastq HEK_CloneZD29_30min_dTAGV1_rep2_PE1.fastq
mv SRR28879488_2.fastq HEK_CloneZD29_30min_dTAGV1_rep2_PE2.fastq
mv SRR28879489_1.fastq HEK_CloneZD29_30min_dTAGV1_rep1_PE1.fastq
mv SRR28879489_2.fastq HEK_CloneZD29_30min_dTAGV1_rep1_PE2.fastq

mv SRR28879490_1.fastq HEK_CloneZD29_30min_control_rep4_PE1.fastq
mv SRR28879490_2.fastq HEK_CloneZD29_30min_control_rep4_PE2.fastq
mv SRR28879491_1.fastq HEK_CloneZD29_30min_control_rep3_PE1.fastq
mv SRR28879491_2.fastq HEK_CloneZD29_30min_control_rep3_PE2.fastq
mv SRR28879492_1.fastq HEK_CloneZD29_30min_control_rep2_PE1.fastq
mv SRR28879492_2.fastq HEK_CloneZD29_30min_control_rep2_PE2.fastq
mv SRR28879493_1.fastq HEK_CloneZD29_30min_control_rep1_PE1.fastq
mv SRR28879493_2.fastq HEK_CloneZD29_30min_control_rep1_PE2.fastq

```


# PRO-seq preparation for analysis

Create directory to hold all scripts and executables:
...
rsync -aP rsync://hgdownload.soe.ucsc.edu/genome/admin/exe/linux.x86_64/twoBitInfo ./
```

Build indices for reference human genome and rDNA: 

```{r engine='bash', eval=F, echo=TRUE}
#! /usr/bin/bash

#SBATCH --job-name=hg38_build
#SBATCH -N 1                    
#SBATCH -n 1                  
#SBATCH -c 32               
#SBATCH -p general
...
export TMPDIR=/scratch/jdong/
cd /home/FCAM/jdong/human38/
echo "Current wd:" $(pwd)
echo "Current node:" $(hostname)

SOFTdir=/scratch/jdong/ZNF143dTAG/software

module load bowtie2
module load genometools/1.5.10
module load ucsc_genome/2012.05.22

# Build index for reference human genome
echo "Time start build hg38:" $(date '+%a %Y-%m-%d %H:%M:%S %Z')
wget https://hgdownload.cse.ucsc.edu/goldenpath/hg38/bigZips/hg38.fa.gz
gunzip hg38.fa.gz
bowtie2-build hg38.fa hg38

# Generate chrom sizes file from hg38 FASTA
echo "Time start make chrom sizes:" $(date '+%a %Y-%m-%d %H:%M:%S %Z')
$SOFTdir/faToTwoBit hg38.fa hg38.2bit
$SOFTdir/twoBitInfo hg38.2bit stdout | sort -k2rn > hg38.chrom.sizes

echo "Time script complete:" $(date '+%a %Y-%m-%d %H:%M:%S %Z')
```
...
export TMPDIR=/scratch/jdong/
cd /home/FCAM/jdong/hum_rDNA
echo "Current wd:" $(pwd)
echo "Current node:" $(hostname)

module load bowtie2

# Build index for reference human rDNA
echo "Time start build human rDNA:" $(date '+%a %Y-%m-%d %H:%M:%S %Z')
...
Use seqOutBias (https://github.com/guertinlab/seqOutBias) to compute mappability for the given read length and the k-mer that corresponds to each possible read alignment position.
This is the most time-consuming step of the seqOutBias command but only needs to be completed once before processing the sequencing data. The files can be reused for future samples if the read size is the same.

Install from source following the instructions in the seqOutBias manual; if you have issues: guertin@uchc.edu

The following code chunk will generate the following output files for our particular read size:

- tallymer=hg38.tal_${read_size}.gtTxt.gz
- table=hg38_${read_size}.4.2.2.tbl

```{r engine='bash', eval=F, echo=TRUE}
mkdir /scratch/jdong/ZNF143dTAG/sOBfiles/
cd /scratch/jdong/ZNF143dTAG/sOBfiles/

# Run seqOutBias in the background with the following script:

#! /usr/bin/bash

#SBATCH --job-name=seqOutBias47
#SBATCH -N 1                  
#SBATCH -n 1                 
#SBATCH -c 36                  
#SBATCH -p general           
...
seqOutBias seqtable $genome --read-size=47

echo "Time script finished:" $(date '+%a %Y-%m-%d %H:%M:%S %Z')
```

Load modules and parse the hg38 annotation file:
...
# Download executables and scripts into a directory in your $PATH variable
cd /scratch/jdong/ZNF143dTAG/software
...
wget https://raw.githubusercontent.com/guertinlab/Nascent_RNA_Methods/main/PRO_normalization_sizefactor 
wget https://raw.githubusercontent.com/guertinlab/Nascent_RNA_Methods/main/normalization_factor.R
wget https://raw.githubusercontent.com/guertinlab/Nascent_RNA_Methods/main/normalize_bedGraph_v2.py

wget https://raw.githubusercontent.com/guertinlab/TSSinference/refs/heads/master/TSSinference.R 
...
chmod +x normalize_bedGraph_v2.py
chmod +x normalization_factor.R
chmod +x PRO_normalization_sizefactor
...
mkdir /scratch/jdong/ZNF143dTAG/PRO/gene_annotations
cd /scratch/jdong/ZNF143dTAG/PRO/gene_annotations

# Get and parse gene annotation file
release=109
...
# get 4 million reads
cat HEK_CloneZD29_30min_control_rep4_PE1.fastq | head -n 16000000 > Z143_UMI_test.fastq
...
cutadapt --cores=1 -m 1 -O 1 -a TGGAATTCTCGGGTGCCAAGG Z143_UMI_test.fastq \
  -o Z143_UMI_test_noadapt.fastq > Z143_UMI_test_cutadapt.txt

tail Z143_UMI_test_cutadapt.txt
# 38	12929	0.0	2	11405 1378 146
# 39	790549	0.0	2	734089 49734 6726
# 40	32720	0.0	2	27144 5117 459
# 41	22664	0.0	2	20571 1569 524
# 42	18347	0.0	2	16929 1113 305
# 43	17051	0.0	2	15752 1023 276
# 44	15301	0.0	2	14121 948 232
# 45	14804	0.0	2	13762 796 246
# 46	17343	0.0	2	16280 866 197
# 47	18093	0.0	2	16797 1005 291
```
...
module load bowtie2
genome_index=/scratch/jdong/ZNF143dTAG/human38/hg38
cd /scratch/jdong/ZNF143dTAG/PRO/FASTQ
...
74.17% overall alignment rate # 1
78.61% overall alignment rate # 2
82.83% overall alignment rate # 3
84.85% overall alignment rate # 4
86.66% overall alignment rate # 5
89.18% overall alignment rate # 6
74.31% overall alignment rate # 7
95.83% overall alignment rate # 8 -- spike
...
#SBATCH --job-name=pro_processing_XXXXXXX
#SBATCH -N 1                  
#SBATCH -n 1                 
#SBATCH -c 24                  
#SBATCH -p general           
...
cd /scratch/jdong/ZNF143dTAG/PRO/FASTQ
echo "Current wd:" $(pwd)
echo "Current node:" $(hostname) $(date '+%a %Y-%m-%d %H:%M:%S %Z')
...
seqOutBias=/home/FCAM/jdong/software/seqOutBias #version 1.4.0
fqdedup=/scratch/jdong/ZNF143dTAG/software/fqdedup
...
sizes=/scratch/jdong/ZNF143dTAG/human38/hg38.chrom.sizes
annotation_prefix=/scratch/jdong/ZNF143dTAG/PRO/gene_annotations/Homo_sapiens.GRCh38.109 
UMI_length=8
read_size=47
cores=24
genome=/scratch/jdong/ZNF143dTAG/human38/hg38.fa
genome_index=/scratch/jdong/ZNF143dTAG/human38/hg38
prealign_rdna_index=/scratch/jdong/ZNF143dTAG/hum_rDNA
tallymer=/scratch/jdong/ZNF143dTAG/sOBfiles/hg38.tal_${read_size}.gtTxt.gz
table=/scratch/jdong/ZNF143dTAG/sOBfiles/hg38_${read_size}.4.2.2.tbl
...
echo 'script complete' $(date '+%a %Y-%m-%d %H:%M:%S %Z')
```
...
file=/scratch/jdong/ZNF143dTAG/pro_processing.sh
...
# check if scripts were populated correctly, then run
for i in pro_processing_*rep*.sh
do
    echo $i
    sbatch $i
done
...
After checking if outputs were produced correctly, remove intermediate files:
...
# PRO-seq normalization 

First, create a text file of the DESeq-generated size factors:
...
Next, run PRO_normalization_sizefactor from Nascent RNA Methods in the directory containing only unnormalized bigWigs:

```{r engine='bash', eval=F, echo=TRUE}
PRO_normalization_sizefactor -c hg38.chrom.sizes -z sizefactorsFile.txt
...
Merge the activated/repressed/matched files together (duplicates okay):

```{r engine='bash', eval=F, echo=TRUE}
cat ZNF143dTAG_*genes* > ZNF143dTAG_act_rep_matches.bed
